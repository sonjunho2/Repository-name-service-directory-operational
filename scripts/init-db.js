...
async function main(){
 ...
 await q(`CREATE TABLE IF NOT EXISTS vendors(id SERIAL PRIMARY KEY, name TEXT NOT NULL, category TEXT NOT NULL, region TEXT NOT NULL, phone TEXT, tags TEXT, description TEXT, image_data TEXT, is_recommended BOOLEAN DEFAULT false, is_premium BOOLEAN DEFAULT false, status TEXT DEFAULT 'active', views INTEGER DEFAULT 0, created_at TIMESTAMP DEFAULT now())`);
 ...
 if(!rows[0].c){
  await q(`INSERT INTO vendors(name,category,region,phone,tags,description,is_recommended,is_premium) VALUES
  ('프리미엄 샘플업체','카페','서울','010-0000-0000','추천,프리미엄','관리자에서 이미지와 정보를 수정하세요.',true,true),
  ('추천 샘플업체','뷰티','부산','010-1111-1111','추천','추천업체 카드 예시입니다.',true,false),
  ('일반 샘플업체','맛집','대구','010-2222-2222','일반','일반업체 카드 예시입니다.',false,false)`);
 }
