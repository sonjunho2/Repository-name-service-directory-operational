...
async function ensureSchema(){
    ...
    await q("CREATE INDEX IF NOT EXISTS idx_flags_user_status_created ON flags(user_id,status,created_at DESC)");
    await q("CREATE INDEX IF NOT EXISTS idx_flags_target_status ON flags(type,target_id,status)");
    ...
    try{
      ...
      const noticeBoard=await noticeMigrationClient.query("SELECT id FROM board_categories WHERE slug='notice' FOR UPDATE");
      const legacyAdmin=await noticeMigrationClient.query("SELECT id FROM users WHERE role='admin' AND status='active' ORDER BY id LIMIT 1");
      if(noticeBoard.rows[0]){
        await noticeMigrationClient.query(`INSERT INTO board_posts(board_id,user_id,title,content,is_pinned,status,created_at,updated_at,legacy_notice_id)
          SELECT $1,$2,n.title,n.content,COALESCE(n.is_pinned,false),'visible',n.created_at,n.created_at,n.id
          FROM notices n
          WHERE NOT EXISTS(SELECT 1 FROM board_posts p WHERE p.legacy_notice_id=n.id)
            AND NOT EXISTS(SELECT 1 FROM board_posts p WHERE p.board_id=$1 AND p.title=n.title AND p.content=n.content AND p.created_at=n.created_at)
          ON CONFLICT (legacy_notice_id) DO NOTHING`,[noticeBoard.rows[0].id,legacyAdmin.rows[0]?.id||null]);
      }
      await noticeMigrationClient.query('COMMIT');
    }catch(error){
      await noticeMigrationClient.query('ROLLBACK');
      throw error;
    }finally{noticeMigrationClient.release();}
...
async function homeData(req){
  ...
  const notices=await q(`SELECT p.id,p.title,p.content,p.is_pinned,p.created_at FROM board_posts p JOIN board_categories b ON b.id=p.board_id WHERE b.slug='notice' AND b.is_active=true AND p.status='visible' ORDER BY p.is_pinned DESC,p.created_at DESC,p.id DESC LIMIT 5`);
...
app.get('/boards',async(req,res)=>{
  try{
    ...
    const reportBoard=boards.find(x=>x.slug==='reports');
...
app.get('/boards/:slug',async(req,res)=>{
  try{
    ...
    if(board.slug==='reports'){
      const user=req.session.user;if(!user)return res.redirect('/login');if(user.role==='admin')return res.redirect('/admin#reports');
      const qText=String(req.query.q||'').trim().slice(0,100),params=[user.id],search=qText?` AND (f.id::text ILIKE $2 OR f.reason ILIKE $2 OR f.content ILIKE $2 OR v.name ILIKE $2 OR r.title ILIKE $2 OR rv.name ILIKE $2)`:'';
      if(qText)params.push(`%${qText}%`);
      const fromSql=` FROM flags f LEFT JOIN vendors v ON f.type='vendor' AND v.id=f.target_id AND ${PUBLIC_VENDOR_SQL} LEFT JOIN reviews r ON f.type='review' AND r.id=f.target_id AND r.status='visible' LEFT JOIN vendors rv ON rv.id=r.vendor_id AND rv.status='active' AND COALESCE(rv.ad_type,'none')<>'none' AND rv.expire_at IS NOT NULL AND rv.expire_at>=CURRENT_DATE WHERE f.user_id=$1${search}`;
      ...
      const rows=await q(`SELECT f.id,f.type,f.target_id,f.reason,f.content,COALESCE(f.status,'new') status,f.admin_memo,f.created_at,f.updated_at,f.processed_at,CASE WHEN f.type='vendor' THEN v.name WHEN f.type='review' THEN r.title END target_name,CASE WHEN f.type='review' THEN rv.name END target_vendor_name,CASE WHEN f.type='vendor' THEN v.id IS NOT NULL WHEN f.type='review' THEN r.id IS NOT NULL AND rv.id IS NOT NULL ELSE false END target_exists,CASE COALESCE(f.status,'new') WHEN 'new' THEN '처리대기' WHEN 'done' THEN '처리완료' WHEN 'rejected' THEN '기각' WHEN 'cancelled' THEN '취소' ELSE f.status END status_label,(COALESCE(f.status,'new')='new') can_cancel${fromSql} ORDER BY f.created_at DESC,f.id DESC LIMIT $${params.length+1} OFFSET $${params.length+2}`,[...params,limit,offset]);
      return res.render('board-list',{mode:'posts',boards:await getBoardCategories(user),board,posts:rows.rows,recentByBoard:{},page,totalPages,qText,canWrite:false,settings:await getSettings()});
    }
...
app.get('/boards/:slug/write',async(req,res)=>{
  ...
  if(board.slug==='reports'){
    const values={type:String(req.query.type||''),target_id:String(req.query.target_id||''),reason:'',content:''};
    const reportTarget=await getReportTarget(values.type,values.target_id);
    return res.status(reportTarget?200:400).render('board-write',{board,settings:await getSettings(),error:reportTarget?null:'업체 또는 이용후기의 신고 버튼을 이용해주세요.',reportTarget,values,vendors:[]});
  }
  if(board.slug==='reviews'){
...
app.post('/boards/:slug/write',login,upload.single('image'),async(req,res)=>{
  try{
    ...
    if(!canWriteBoard(req.session.user,board))return res.status(403).send('글쓰기 권한이 없습니다.');
    if(board.slug==='reports'){
      const values={type:String(req.body.type||''),target_id:String(req.body.target_id||''),reason:String(req.body.reason||''),content:String(req.body.content||'')};
      try{const saved=await createFlagForUser({userId:req.session.user.id,type:values.type,targetId:values.target_id,reason:values.reason,content:values.content});return res.redirect(`/boards/reports/${saved.id}`);}
      catch(e){const reportTarget=await getReportTarget(values.type,values.target_id);return res.status(e.status||500).render('board-write',{board,settings:await getSettings(),error:reportErrorMessage(e),reportTarget,values,vendors:[]});}
    }
    if(board.slug==='reviews'){
...
app.get('/boards/:slug/:id',async(req,res)=>{
  try{
    ...
    if(String(req.params.slug||'').toLowerCase()==='reports'){
      const user=req.session.user;if(!user)return res.redirect('/login');if(user.role==='admin')return res.redirect('/admin#reports');
      const found=await q(`SELECT f.id,f.user_id,f.type,f.target_id,f.reason,f.content,COALESCE(f.status,'new') status,f.admin_memo,f.created_at,f.updated_at,f.processed_at,CASE WHEN f.type='vendor' THEN v.name WHEN f.type='review' THEN r.title END target_name,CASE WHEN f.type='review' THEN rv.name END target_vendor_name,CASE WHEN f.type='vendor' THEN v.id IS NOT NULL WHEN f.type='review' THEN r.id IS NOT NULL AND rv.id IS NOT NULL ELSE false END target_exists,CASE COALESCE(f.status,'new') WHEN 'new' THEN '처리대기' WHEN 'done' THEN '처리완료' WHEN 'rejected' THEN '기각' WHEN 'cancelled' THEN '취소' ELSE f.status END status_label,(COALESCE(f.status,'new')='new') can_cancel FROM flags f LEFT JOIN vendors v ON f.type='vendor' AND v.id=f.target_id AND ${PUBLIC_VENDOR_SQL} LEFT JOIN reviews r ON f.type='review' AND r.id=f.target_id AND r.status='visible' LEFT JOIN vendors rv ON rv.id=r.vendor_id AND rv.status='active' AND COALESCE(rv.ad_type,'none')<>'none' AND rv.expire_at IS NOT NULL AND rv.expire_at>=CURRENT_DATE WHERE f.id=$1 AND f.user_id=$2`,[id,user.id]);
      const post=found.rows[0];if(!post)return res.status(404).send('신고내역을 찾을 수 없습니다.');
      const board=(await q("SELECT * FROM board_categories WHERE slug='reports' AND is_active=true")).rows[0];if(!board)return res.status(404).send('게시판을 찾을 수 없습니다.');
      post.title=`신고 #${post.id}`;return res.render('board-post',{board,post,comments:[],canWrite:false,settings:await getSettings()});
    }
...
async function getReportTarget(type,targetId){
  const safeType=String(type||'').trim(),id=Number(targetId);
  if(!['vendor','review'].includes(safeType)||!Number.isInteger(id)||id<=0)return null;
  if(safeType==='vendor'){
    const r=await q(`SELECT v.id,v.name,v.category,v.region FROM vendors v WHERE v.id=$1 AND ${PUBLIC_VENDOR_SQL}`,[id]),x=r.rows[0];
    return x?{type:'vendor',target_id:x.id,target_name:x.name,target_subtitle:[x.region,x.category].filter(Boolean).join(' · ')}:null;
  }
  const r=await q(`SELECT r.id,r.title,r.vendor_id,r.rating,v.name vendor_name FROM reviews r JOIN vendors v ON v.id=r.vendor_id WHERE r.id=$1 AND r.status='visible' AND ${PUBLIC_VENDOR_SQL}`,[id]),x=r.rows[0];
  return x?{type:'review',target_id:x.id,target_name:x.title,target_subtitle:[x.vendor_name,x.rating?`★${x.rating}`:''].filter(Boolean).join(' · ')}:null;
}
function reportError(code,status=400,existingId=null){const e=new Error(code);e.code=code;e.status=status;e.existingId=existingId;return e;}
function reportErrorMessage(e){return {invalid_report:'신고 사유와 상세 내용을 확인해주세요.',target_not_found:'신고 가능한 공개 대상을 찾을 수 없습니다.',report_duplicate:'같은 대상에 처리 대기 중인 신고가 있습니다.'}[e.code]||'신고를 접수하지 못했습니다.';}
async function createFlagForUser({userId,type,targetId,reason,content}){
  const uid=Number(userId),safeType=String(type||'').trim(),id=Number(targetId),safeReason=String(reason||'').trim(),safeContent=String(content||'').trim();
  if(!Number.isInteger(uid)||uid<=0||!['vendor','review'].includes(safeType)||!Number.isInteger(id)||id<=0||safeReason.length<2||safeReason.length>50||safeContent.length<5||safeContent.length>2000)throw reportError('invalid_report');
  const active=await q("SELECT id FROM users WHERE id=$1 AND status='active'",[uid]);if(!active.rows[0])throw reportError('invalid_report');
  const target=await getReportTarget(safeType,id);if(!target)throw reportError('target_not_found',404);
  const duplicate=await q("SELECT id FROM flags WHERE user_id=$1 AND type=$2 AND target_id=$3 AND COALESCE(status,'new')='new' LIMIT 1",[uid,safeType,id]);
  if(duplicate.rows[0])throw reportError('report_duplicate',409,duplicate.rows[0].id);
  const saved=await q("INSERT INTO flags(user_id,type,target_id,reason,content,status,created_at,updated_at) VALUES($1,$2,$3,$4,$5,'new',now(),now()) RETURNING id",[uid,safeType,id,safeReason,safeContent]);
  const flagId=saved.rows[0].id;
  try{await adminNotify('report_new','새 신고가 접수되었습니다',`${safeType==='vendor'?'업체 신고':'후기 신고'}: ${target.target_name}${target.target_subtitle?' · '+target.target_subtitle:''} · ${safeReason}`,'/admin#reports');}catch(e){console.error('report notification failed',e.message);}
  return {id:flagId,target};
}
...
app.post('/api/flag',login,async(req,res)=>{try{const saved=await createFlagForUser({userId:req.session.user.id,type:req.body.type,targetId:req.body.target_id,reason:req.body.reason,content:req.body.content});res.json({ok:true,id:saved.id,redirect:`/boards/reports/${saved.id}`});}catch(e){const code=['invalid_report','target_not_found','report_duplicate'].includes(e.code)?e.code:'report_create_failed';res.status(e.status&&e.status<500?e.status:500).json({ok:false,error:code,id:e.existingId||undefined});}});

app.post('/boards/reports/:id/cancel',login,async(req,res)=>{const id=Number(req.params.id);if(!Number.isInteger(id)||id<=0)return res.status(404).send('신고내역을 찾을 수 없습니다.');const r=await q("UPDATE flags SET status='cancelled',updated_at=now() WHERE id=$1 AND user_id=$2 AND COALESCE(status,'new')='new' RETURNING id",[id,req.session.user.id]);if(!r.rows[0])return res.status(409).send('이미 처리되었거나 취소할 수 없는 신고입니다.');res.redirect(`/boards/reports/${id}`);});
...
app.get('/admin/api/vendors',admin,async(req,res)=>{
  ...
  const adType=String(req.query.quick||req.query.ad_type||'').trim();
  const orderBy={latest:'v.id DESC',oldest:'v.id ASC',name:'v.name ASC,v.id DESC',expire:'v.expire_at ASC NULLS LAST,v.id DESC'}[String(req.query.sort||'latest')]||'v.id DESC';
  ...
  try{
    ...
    const summary=(await q(`SELECT COUNT(*)::int total,
      ...
      COUNT(*) FILTER (WHERE status='active' AND COALESCE(ad_type,'none')<>'none' AND expire_at BETWEEN CURRENT_DATE AND CURRENT_DATE+INTERVAL '7 days')::int expiring FROM vendors`)).rows[0]||{};
    ...
    return res.json({ok:true,page,limit,total,totalPages:Math.max(1,Math.ceil(total/limit)),rows:apiRows,summary:{total:Number(summary.total||0),active:Number(summary.active||0),pending:Number(summary.pending||0),hiddenOrExpired:Number(summary.hiddenOrExpired||0),expiring:Number(summary.expiring||0)}});
...
app.get('/admin/api/users',admin,async(req,res)=>{
  try{
    res.setHeader('Cache-Control','no-store');
    const where=[],params=[];
    const qText=String(req.query.q||'').trim().slice(0,100),role=String(req.query.role||'').trim(),status=String(req.query.status||'').trim(),link=String(req.query.vendor_link||'').trim(),memberScope=String(req.query.member_scope||'').trim();
    ...
    if(memberScope==='general')where.push("u.role<>'admin' AND COALESCE(u.is_vendor,false)=false AND u.vendor_id IS NULL");
    ...
    const rows=await q(`SELECT u.id,u.username,u.nickname,u.role,u.status,COALESCE(u.is_vendor,false) is_vendor,u.vendor_id,u.created_at,u.last_login_at,
...
app.get('/admin/api/banners',admin,async(req,res)=>{
  try{
    res.setHeader('Cache-Control','no-store');const params=[],where=[],qText=String(req.query.q||'').trim().slice(0,100),type=String(req.query.type||req.query.source||'all'),status=String(req.query.status||'all'),image=String(req.query.image||'all');
    const baseSql=` FROM (SELECT b.id banner_id,b.id::text display_id,b.id source_id,b.title,b.subtitle,b.link_url,b.position,b.sort_order,b.is_active,b.vendor_id,b.created_at,CASE WHEN b.vendor_id IS NULL THEN COALESCE(b.image_data,'')<>'' ELSE COALESCE(v.image_data,b.image_data,'')<>'' END image_exists,v.name vendor_name,v.region vendor_region,v.category vendor_category,v.status vendor_status,v.ad_type vendor_ad_type,v.expire_at vendor_expire_at,v.banner_active vendor_banner_active,v.banner_until vendor_banner_until,CASE WHEN b.vendor_id IS NULL THEN 'direct' ELSE 'vendor' END source_type,CASE WHEN b.vendor_id IS NULL THEN '관리자 직접' ELSE '업체연동' END source_label,CASE WHEN b.vendor_id IS NULL THEN CASE WHEN COALESCE(b.image_data,'')='' THEN 'no_image' WHEN b.is_active THEN 'active' ELSE 'hidden' END WHEN COALESCE(v.image_data,b.image_data,'')='' THEN 'no_image' WHEN v.status<>'active' THEN 'vendor_hidden' WHEN COALESCE(v.ad_type,'none')='none' THEN 'no_ad' WHEN v.expire_at<CURRENT_DATE OR v.banner_until<CURRENT_DATE THEN 'expired' WHEN COALESCE(v.banner_active,false) AND v.expire_at>=CURRENT_DATE AND v.banner_until>=CURRENT_DATE THEN 'active' ELSE 'hidden' END display_status FROM banners b LEFT JOIN vendors v ON v.id=b.vendor_id UNION ALL SELECT NULL banner_id,('vendor-'||v.id) display_id,v.id source_id,v.name title,'' subtitle,'' link_url,'premium' position,0 sort_order,true is_active,v.id vendor_id,v.created_at,COALESCE(v.image_data,'')<>'' image_exists,v.name vendor_name,v.region vendor_region,v.category vendor_category,v.status vendor_status,v.ad_type vendor_ad_type,v.expire_at vendor_expire_at,v.banner_active vendor_banner_active,v.banner_until vendor_banner_until,'vendor' source_type,'업체연동' source_label,CASE WHEN COALESCE(v.image_data,'')='' THEN 'no_image' WHEN v.status<>'active' THEN 'vendor_hidden' WHEN COALESCE(v.ad_type,'none')='none' THEN 'no_ad' WHEN v.expire_at<CURRENT_DATE OR v.banner_until<CURRENT_DATE THEN 'expired' WHEN COALESCE(v.banner_active,false) AND v.expire_at>=CURRENT_DATE AND v.banner_until>=CURRENT_DATE THEN 'active' ELSE 'hidden' END display_status FROM vendors v WHERE COALESCE(v.banner_active,false)=true AND NOT EXISTS(SELECT 1 FROM banners bx WHERE bx.vendor_id=v.id)) x`;
    if(qText){params.push(`%${qText}%`);const p=`$${params.length}`;where.push(`(x.display_id ILIKE ${p} OR x.vendor_id::text ILIKE ${p} OR x.title ILIKE ${p} OR x.vendor_name ILIKE ${p} OR x.vendor_region ILIKE ${p} OR x.vendor_category ILIKE ${p} OR x.link_url ILIKE ${p})`);}if(['direct','vendor'].includes(type)){params.push(type);where.push(`x.source_type=$${params.length}`);}if(status==='active')where.push("x.display_status='active'");else if(status==='hidden')where.push("x.display_status IN ('hidden','vendor_hidden','no_ad')");else if(status==='expired')where.push("x.display_status='expired'");else if(status==='warning')where.push("x.display_status IN ('no_image','vendor_hidden','no_ad')");if(image==='exists')where.push('x.image_exists=true');else if(image==='missing')where.push('x.image_exists=false');
    const whereSql=where.length?' WHERE '+where.join(' AND '):'',orderBy={default:'x.sort_order,x.created_at DESC,x.display_id DESC',latest:'x.created_at DESC,x.display_id DESC',order:'x.sort_order,x.created_at DESC',active:"CASE WHEN x.display_status='active' THEN 0 ELSE 1 END,x.sort_order",expiring:'x.vendor_banner_until ASC NULLS LAST,x.display_id DESC',vendor:'x.vendor_name ASC NULLS LAST',title:'x.title ASC NULLS LAST'}[String(req.query.sort||'default')]||'x.sort_order,x.created_at DESC';const {page,limit,offset}=adminPageParams(req),rows=await q(`SELECT x.*,CASE x.display_status WHEN 'active' THEN '정상노출' WHEN 'hidden' THEN '숨김' WHEN 'expired' THEN '만료' WHEN 'vendor_hidden' THEN '업체 비노출' WHEN 'no_image' THEN '이미지 없음' WHEN 'no_ad' THEN '광고상품 없음' ELSE x.display_status END display_status_label,CASE WHEN x.source_type='direct' THEN '외부/내부 링크' ELSE '업체정보 팝업' END click_behavior,(x.source_type='direct') can_edit,(x.source_type='direct') can_delete,(x.source_type='vendor' AND x.vendor_banner_until BETWEEN CURRENT_DATE AND CURRENT_DATE+7) expiring${baseSql}${whereSql} ORDER BY ${orderBy} LIMIT $${params.length+1} OFFSET $${params.length+2}`,[...params,limit,offset]),count=await q(`SELECT COUNT(*)${baseSql}${whereSql}`,params),total=Number(count.rows[0]?.count||0),summary=await q(`SELECT COUNT(*)::int total,COUNT(*) FILTER(WHERE display_status='active')::int active,COUNT(*) FILTER(WHERE display_status<>'active')::int hidden,COUNT(*) FILTER(WHERE source_type='direct')::int direct,COUNT(*) FILTER(WHERE source_type='vendor')::int vendor,COUNT(*) FILTER(WHERE source_type='vendor' AND vendor_banner_until BETWEEN CURRENT_DATE AND CURRENT_DATE+7)::int expiring,COUNT(*) FILTER(WHERE image_exists=false)::int "missingImage"${baseSql}`);return res.json({ok:true,rows:rows.rows,page,limit,total,totalPages:Math.max(1,Math.ceil(total/limit)),summary:summary.rows[0]||{}});
  }catch(e){console.error('admin banners api failed',e);return res.status(500).json({ok:false,error:e.message||'admin_banners_load_failed'});}
});
app.get('/admin/api/notices',admin,async(req,res)=>{
  try{
    res.setHeader('Cache-Control','no-store');
    const params=[],where=[],qText=String(req.query.q||'').trim().slice(0,100),pinned=String(req.query.pinned??req.query.is_pinned??'all').trim();
    if(qText){params.push(`%${qText}%`);const value=`$${params.length}`;where.push(`(p.id::text ILIKE ${value} OR p.title ILIKE ${value} OR p.content ILIKE ${value})`);}
    if(['true','1','pinned'].includes(pinned)){params.push(true);where.push(`p.is_pinned=$${params.length}`);}else if(['false','0','normal'].includes(pinned)){params.push(false);where.push(`p.is_pinned=$${params.length}`);}
    const whereSql=where.length?' WHERE '+where.join(' AND '):'',orderBy={default:'n.created_at DESC,n.id DESC',latest:'n.created_at DESC,n.id DESC',oldest:'n.created_at ASC,n.id ASC',pinned:'n.is_pinned DESC,n.created_at DESC,n.id DESC',normal:'n.is_pinned ASC,n.created_at DESC,n.id DESC',title:'n.title ASC,n.id DESC'}[String(req.query.sort||'latest')]||'n.created_at DESC,n.id DESC';
    const noticeFrom=" FROM board_posts p JOIN board_categories b ON b.id=p.board_id AND b.slug='notice'",noticeOrder=orderBy.replaceAll('n.','p.');
    const {page,limit,offset}=adminPageParams(req),rows=await q(`SELECT p.id,p.title,p.content,p.is_pinned,p.created_at,p.updated_at,p.views,(SELECT COUNT(*)::int FROM board_comments c WHERE c.post_id=p.id) comment_count,(COALESCE(p.image_data,'')<>'') has_image${noticeFrom}${whereSql} ORDER BY ${noticeOrder} LIMIT $${params.length+1} OFFSET $${params.length+2}`,[...params,limit,offset]);
    const count=await q(`SELECT COUNT(*)${noticeFrom}${whereSql}`,params),total=Number(count.rows[0]?.count||0),summary=await q(`SELECT COUNT(*)::int total,COUNT(*) FILTER (WHERE p.is_pinned=true)::int pinned,COUNT(*) FILTER (WHERE COALESCE(p.is_pinned,false)=false)::int normal,COUNT(*) FILTER (WHERE p.created_at>=CURRENT_TIMESTAMP-INTERVAL '7 days')::int recent${noticeFrom}`);
  ...
  }catch(e){console.error('admin notices api failed',e);return res.status(500).json({ok:false,error:e.message||'admin_notices_load_failed'});}
});
app.get('/admin/api/reviews',admin,async(req,res)=>{
  try{
    res.setHeader('Cache-Control','no-store');
    const params=[],where=[],qText=String(req.query.q||'').trim().slice(0,100),status=String(req.query.status||'all'),rating=String(req.query.rating||'all'),report=String(req.query.report||'all');
    const reports=`LEFT JOIN LATERAL (SELECT COUNT(*)::int report_count,COUNT(*) FILTER (WHERE COALESCE(f.status,'new')='new')::int pending_report_count,MAX(f.created_at) latest_report_at FROM flags f WHERE f.type='review' AND f.target_id=r.id) rf ON true`;
    const fromSql=` FROM reviews r LEFT JOIN vendors v ON v.id=r.vendor_id LEFT JOIN users u ON u.id=r.user_id ${reports}`;
    if(qText){params.push(`%${qText}%`);const p=`$${params.length}`;where.push(`(r.id::text ILIKE ${p} OR r.title ILIKE ${p} OR r.content ILIKE ${p} OR v.name ILIKE ${p} OR v.region ILIKE ${p} OR v.category ILIKE ${p} OR u.username ILIKE ${p} OR u.nickname ILIKE ${p})`);}
    if(['visible','hidden','deleted'].includes(status)){params.push(status);where.push(`r.status=$${params.length}`);}
    if(['5','4','3'].includes(rating)){params.push(Number(rating));where.push(`r.rating=$${params.length}`);}
    else if(rating==='low')where.push('r.rating BETWEEN 1 AND 2');
    else if(rating==='high')where.push('r.rating BETWEEN 4 AND 5');
    if(report==='reported')where.push('COALESCE(rf.report_count,0)>0');
    else if(report==='pending')where.push('COALESCE(rf.pending_report_count,0)>0');
    else if(report==='none')where.push('COALESCE(rf.report_count,0)=0');
    const whereSql=where.length?' WHERE '+where.join(' AND '):'';
    ...
    return res.json({ok:true,rows:rows.rows,page,limit,total,totalPages:Math.max(1,Math.ceil(total/limit)),summary:summary.rows[0]||{}});
  }catch(e){console.error('admin reviews api failed',e);return res.status(500).json({ok:false,error:e.message||'admin_reviews_load_failed'});}
});
...
app.post('/admin/reports/:id/done',admin,async(req,res)=>runAdminAction(req,res,'/admin#reports',async()=>{
  ...
  const r=await q("UPDATE flags SET status='done', admin_memo=$1, processed_at=now(),updated_at=now() WHERE id=$2 AND COALESCE(status,'new')='new' RETURNING id,user_id,type,target_id",[memo,req.params.id]);
  ...
  if(row.user_id)await userNotify(row.user_id,'report_done','신고 처리가 완료되었습니다',`신고 대상 ${row.type}:${row.target_id} · 처리완료 · ${memo.slice(0,200)}`,`/boards/reports/${row.id}`);
}));
app.post('/admin/reports/:id/reject',admin,async(req,res)=>runAdminAction(req,res,'/admin#reports',async()=>{
  ...
  const r=await q("UPDATE flags SET status='rejected', admin_memo=$1, processed_at=now(),updated_at=now() WHERE id=$2 AND COALESCE(status,'new')='new' RETURNING id,user_id,type,target_id",[memo,req.params.id]);
  if(!r.rows[0]){const e=new Error('처리할 신고를 찾을 수 없거나 이미 처리되었습니다.');e.status=409;throw e;}
  const row=r.rows[0];
  await logAdmin(req,'신고 기각','report',row.id,`${row.type}:${row.target_id} / ${memo}`);
  if(row.user_id)await userNotify(row.user_id,'report_rejected','신고 검토 결과가 등록되었습니다',`신고 대상 ${row.type}:${row.target_id} · 기각 · ${memo.slice(0,200)}`,`/boards/reports/${row.id}`);
}));
...
const [users,vendors,banners,reviews,events,notices,inquiries,flags,vendorRequests,bannerRequests,adRequests,adminLogs,paymentLogs,settings]=await Promise.all([q('SELECT u.id,u.username,u.nickname,u.role,u.status,u.is_vendor,u.vendor_id,u.created_at FROM users u ORDER BY u.id DESC LIMIT 300'),q(`SELECT v.id,v.name,v.category,v.region,v.phone,v.kakao_url,v.business_hours,v.description,v.tags,v.status,v.ad_type,v.expire_at,v.banner_active,v.banner_until,v.views,v.created_at,v.is_premium,v.is_recommended,v.membership_type,v.sns_url,v.line_url,v.telegram_url,v.holiday_info,v.ad_until,v.scheduled_membership_type,v.scheduled_banner_active,v.scheduled_change_at,v.scheduled_change_note,
  ...
  const [listCounts,bannerCounts,adRequestCounts,bannerRequestCounts]=await Promise.all([
    q(`SELECT
      ...
      (SELECT COUNT(*) FROM board_posts p JOIN board_categories b ON b.id=p.board_id WHERE b.slug='notice')::int notices_total,
      (SELECT COUNT(*) FROM board_posts p JOIN board_categories b ON b.id=p.board_id WHERE b.slug='notice' AND COALESCE(p.is_pinned,false)=true)::int notices_pinned,
...
app.post('/admin/users/:id/update',admin,upload.none(),async(req,res)=>{
  ...
  if(user.role==='admin')return res.status(403).json({ok:false,error:'admin_account_managed_in_settings'});
...
app.post('/admin/delete/users/:id',admin,async(req,res)=>runAdminAction(req,res,'/admin#users',async()=>{
  ...
  if(u.rows[0].role==='admin'){const error=new Error('admin_account_managed_in_settings');error.status=403;throw error;}
...
app.post('/admin/link-user-vendor',admin,upload.none(),async(req,res)=>{
  ...
  if(u.rows[0].role==='admin'){
    if(req.get('x-requested-with'))return res.status(403).json({ok:false,error:'admin_account_managed_in_settings'});
    return res.status(403).send('admin_account_managed_in_settings');
  }
...
app.post('/admin/users/:id/unlink-vendor',admin,async(req,res)=>runAdminAction(req,res,'/admin#users',async()=>{
  ...
  if(user.role==='admin'){const error=new Error('admin_account_managed_in_settings');error.status=403;throw error;}
...
function adminAccountJsonError(res,status,error){return res.status(status).json({ok:false,error});}
function validAdminUsername(value){return /^[a-zA-Z0-9_]{4,30}$/.test(String(value||''));}
async function findManagedAdmin(id,client={query:(sql,params)=>q(sql,params)},lock=false){
  const r=await client.query(`SELECT id,username,nickname,status,password_hash FROM users WHERE id=$1 AND role='admin' AND status<>'deleted'${lock?' FOR UPDATE':''}`,[id]);
...
app.post('/admin/settings/admin-accounts/create',admin,upload.none(),async(req,res)=>{
  const username=String(req.body.username||'').trim(),nickname=String(req.body.nickname||'').trim(),password=String(req.body.password||''),confirm=String(req.body.password_confirm||'');
  if(!validAdminUsername(username))return adminAccountJsonError(res,400,'invalid_username');
  if(!nickname||nickname.length>50)return adminAccountJsonError(res,400,'invalid_nickname');
  if(password.length<8)return adminAccountJsonError(res,400,'password_too_short');
  if(password!==confirm)return adminAccountJsonError(res,400,'password_mismatch');
  try{
    const h=await bcrypt.hash(password,10);
    const saved=await q("INSERT INTO users(username,nickname,password_hash,role,status,is_vendor,vendor_id) VALUES($1,$2,$3,'admin','active',false,NULL) RETURNING id,username,nickname",[username,nickname,h]);
    const row=saved.rows[0];await logAdmin(req,'관리자 추가','admin_account',row.id,`${row.username} / ${row.nickname}`);return res.json({ok:true,id:row.id});
  }catch(e){if(e?.code==='23505')return adminAccountJsonError(res,409,'username_already_exists');console.error('admin account create failed',e);return adminAccountJsonError(res,500,'admin_account_create_failed');}
});
app.post('/admin/settings/admin-accounts/:id/update',admin,upload.none(),async(req,res)=>{
  const id=parseInt(req.params.id||0,10),username=String(req.body.username||'').trim(),nickname=String(req.body.nickname||'').trim();
  if(!validAdminUsername(username))return adminAccountJsonError(res,400,'invalid_username');if(!nickname||nickname.length>50)return adminAccountJsonError(res,400,'invalid_nickname');
  const old=await findManagedAdmin(id);if(!old)return adminAccountJsonError(res,404,'admin_not_found');
  try{const updated=await q("UPDATE users SET username=$1,nickname=$2 WHERE id=$3 AND role='admin' AND status<>'deleted' RETURNING id,username,nickname",[username,nickname,id]);const row=updated.rows[0];if(!row)return adminAccountJsonError(res,404,'admin_not_found');if(id===Number(req.session.user.id)){req.session.user.username=row.username;req.session.user.nickname=row.nickname;}await logAdmin(req,'관리자 정보 수정','admin_account',id,`${old.username} → ${row.username} / ${old.nickname} → ${row.nickname}`);return res.json({ok:true});}catch(e){if(e?.code==='23505')return adminAccountJsonError(res,409,'username_already_exists');console.error('admin account update failed',e);return adminAccountJsonError(res,500,'admin_account_update_failed');}
});
app.post('/admin/settings/admin-accounts/:id/password',admin,upload.none(),async(req,res)=>{
  const id=parseInt(req.params.id||0,10),password=String(req.body.new_password||''),confirm=String(req.body.new_password_confirm||''),actorPassword=String(req.body.actor_password||'');
  if(password.length<8)return adminAccountJsonError(res,400,'password_too_short');if(password!==confirm)return adminAccountJsonError(res,400,'password_mismatch');
  const target=await findManagedAdmin(id);if(!target)return adminAccountJsonError(res,404,'admin_not_found');const actor=await q("SELECT password_hash FROM users WHERE id=$1 AND role='admin' AND status='active'",[req.session.user.id]);if(!actor.rows[0]||!await bcrypt.compare(actorPassword,actor.rows[0].password_hash))return adminAccountJsonError(res,403,'invalid_actor_password');
  await q("UPDATE users SET password_hash=$1 WHERE id=$2 AND role='admin' AND status<>'deleted'",[await bcrypt.hash(password,10),id]);await logAdmin(req,'관리자 비밀번호 변경','admin_account',id,'관리자 비밀번호 변경 완료');return res.json({ok:true});
});
app.post('/admin/settings/admin-accounts/:id/status',admin,upload.none(),async(req,res)=>{
  const id=parseInt(req.params.id||0,10),status=String(req.body.status||'');if(!['active','suspended'].includes(status))return adminAccountJsonError(res,400,'invalid_status');
  const client=await pool.connect();let old;
  try{await client.query('BEGIN');old=await findManagedAdmin(id,client,true);if(!old){await client.query('ROLLBACK');return adminAccountJsonError(res,404,'admin_not_found');}const active=await client.query("SELECT id FROM users WHERE role='admin' AND status='active' FOR UPDATE");if(status==='suspended'&&id===Number(req.session.user.id)){await client.query('ROLLBACK');return adminAccountJsonError(res,400,'self_suspend_forbidden');}if(status==='suspended'&&old.status==='active'&&active.rows.length<=1){await client.query('ROLLBACK');return adminAccountJsonError(res,409,'last_active_admin');}await client.query("UPDATE users SET status=$1 WHERE id=$2 AND role='admin' AND status<>'deleted'",[status,id]);await client.query('COMMIT');}catch(e){try{await client.query('ROLLBACK');}catch(_){}console.error('admin account status failed',e);return adminAccountJsonError(res,500,'admin_account_status_failed');}finally{client.release();}
  await logAdmin(req,status==='active'?'관리자 활성화':'관리자 사용중지','admin_account',id,old.username);return res.json({ok:true});
});
app.post('/admin/settings/admin-accounts/:id/delete',admin,upload.none(),async(req,res)=>{
  const id=parseInt(req.params.id||0,10),actorPassword=String(req.body.actor_password||''),confirm=String(req.body.confirm_text||'');if(confirm!=='관리자삭제')return adminAccountJsonError(res,400,'invalid_confirm_text');if(id===Number(req.session.user.id))return adminAccountJsonError(res,400,'self_delete_forbidden');
  const actor=await q("SELECT password_hash FROM users WHERE id=$1 AND role='admin' AND status='active'",[req.session.user.id]);if(!actor.rows[0]||!await bcrypt.compare(actorPassword,actor.rows[0].password_hash))return adminAccountJsonError(res,403,'invalid_actor_password');
  const replacement=await bcrypt.hash(crypto.randomBytes(32).toString('hex'),10),client=await pool.connect();let old;
  try{await client.query('BEGIN');old=await findManagedAdmin(id,client,true);if(!old){await client.query('ROLLBACK');return adminAccountJsonError(res,404,'admin_not_found');}const active=await client.query("SELECT id FROM users WHERE role='admin' AND status='active' FOR UPDATE");if(old.status==='active'&&active.rows.length<=1){await client.query('ROLLBACK');return adminAccountJsonError(res,409,'last_active_admin');}const random=crypto.randomBytes(4).toString('hex'),anonymous=`deleted_admin_${id}_${random}`.slice(0,30);await client.query("UPDATE users SET username=$1,nickname='삭제된 관리자',status='deleted',is_vendor=false,vendor_id=NULL,password_hash=$2 WHERE id=$3 AND role='admin'",[anonymous,replacement,id]);await client.query('COMMIT');}catch(e){try{await client.query('ROLLBACK');}catch(_){}console.error('admin account delete failed',e);return adminAccountJsonError(res,500,'admin_account_delete_failed');}finally{client.release();}
  await logAdmin(req,'관리자 삭제','admin_account',id,`${old.username} / ${old.nickname}`);return res.json({ok:true});
});
app.post('/admin/settings/admin-account',admin,upload.none(),async(req,res)=>{
  ...
  const password=(req.body.password||'').trim(),currentPassword=String(req.body.current_password||'');
  ...
  try{
    let updated;
    if(password){
      const actor=await q("SELECT password_hash FROM users WHERE id=$1 AND role='admin' AND status='active'",[req.session.user.id]);
      if(!actor.rows[0]||!await bcrypt.compare(currentPassword,actor.rows[0].password_hash))return sendFail(req,res,403,'invalid_actor_password','/admin#settings');
      const h=await bcrypt.hash(password,10);
    ...
    await logAdmin(req,'관리자 정보 수정','admin_account',saved.id,saved.username);
    return sendOk(req,res,'/admin#settings');
  }catch(e){
...
app.post('/admin/banner',admin,upload.single('image'),async(req,res)=>runAdminAction(req,res,'/admin#banners',async()=>{const im=img(req.file),title=String(req.body.title||'').trim().slice(0,200);if(!title){const e=new Error('배너 제목을 입력해주세요.');e.status=400;throw e;}if(req.body.id){const id=parseInt(req.body.id||0,10);let p=[title,req.body.subtitle,req.body.link_url,req.body.position||'premium',req.body.sort_order||0,!!req.body.is_active,id];const r=await q(`UPDATE banners SET title=$1,subtitle=$2,link_url=$3,position=$4,sort_order=$5,is_active=$6 ${im?', image_data=$8':''} WHERE id=$7 AND vendor_id IS NULL RETURNING id`,im?[...p,im]:p);if(!r.rows[0]){const e=new Error('수정 가능한 관리자 직접 배너를 찾을 수 없습니다.');e.status=409;throw e;}await logAdmin(req,'배너 수정','banner',id,title);return {id};}const r=await q('INSERT INTO banners(title,subtitle,link_url,position,sort_order,is_active,image_data) VALUES($1,$2,$3,$4,$5,$6,$7) RETURNING id',[title,req.body.subtitle,req.body.link_url,req.body.position||'premium',req.body.sort_order||0,!!req.body.is_active,im]);await logAdmin(req,'배너 등록','banner',r.rows[0].id,title);return {id:r.rows[0].id};}));
app.post('/admin/delete/notices/:id',admin,async(req,res)=>runAdminAction(req,res,'/admin#notices',async()=>{const id=parseInt(req.params.id||0,10);if(!id){const e=new Error('공지 ID가 올바르지 않습니다.');e.status=400;throw e;}const r=await q("DELETE FROM board_posts p USING board_categories b WHERE p.id=$1 AND p.board_id=b.id AND b.slug='notice' RETURNING p.id,p.title",[id]);if(!r.rows[0]){const e=new Error('삭제할 공지를 찾을 수 없습니다.');e.status=404;throw e;}await logAdmin(req,'공지 삭제','notice',id,r.rows[0].title||'');}));
app.post('/admin/user',admin,upload.none(),async(req,res)=>{
  ...
  if(user.role==='admin')return wantsJson(req)?res.status(403).json({ok:false,error:'admin_account_managed_in_settings'}):res.status(403).send('admin_account_managed_in_settings');
  const hasNickname=Object.prototype.hasOwnProperty.call(req.body,'nickname');
  const nickname=hasNickname&&String(req.body.nickname||'').trim()?String(req.body.nickname).trim().slice(0,50):user.nickname;
  const role=user.role;
  const status=['active','blocked','suspended','inactive'].includes(req.body.status)?req.body.status:user.status;
...
app.post('/admin/notice',admin,upload.none(),async(req,res)=>runAdminAction(req,res,'/admin#notices',async()=>{const title=String(req.body.title||'').trim(),content=String(req.body.content||'').trim(),isPinned=!!req.body.is_pinned,idText=String(req.body.id||'').trim();if(!title||title.length>200){const e=new Error('공지 제목은 1자 이상 200자 이하로 입력해주세요.');e.status=400;throw e;}if(!content||content.length>20000){const e=new Error('공지 내용은 1자 이상 20,000자 이하로 입력해주세요.');e.status=400;throw e;}if(idText){const id=parseInt(idText,10);if(!id||String(id)!==idText){const e=new Error('공지 ID가 올바르지 않습니다.');e.status=400;throw e;}const r=await q("UPDATE board_posts p SET title=$1,content=$2,is_pinned=$3,updated_at=now() FROM board_categories b WHERE p.id=$4 AND p.board_id=b.id AND b.slug='notice' RETURNING p.id",[title,content,isPinned,id]);if(!r.rows[0]){const e=new Error('수정할 공지를 찾을 수 없습니다.');e.status=404;throw e;}await logAdmin(req,'공지 수정','notice',id,`${title} / 상단고정:${isPinned?'Y':'N'}`);return {id,mode:'updated'};}const board=await q("SELECT id FROM board_categories WHERE slug='notice' AND is_active=true");if(!board.rows[0]){const e=new Error('공지사항 게시판을 찾을 수 없습니다.');e.status=409;throw e;}const r=await q("INSERT INTO board_posts(board_id,user_id,title,content,is_pinned,status,created_at,updated_at) VALUES($1,$2,$3,$4,$5,'visible',now(),now()) RETURNING id",[board.rows[0].id,req.session.user.id,title,content,isPinned]);await logAdmin(req,'공지 등록','notice',r.rows[0].id,title);return {id:r.rows[0].id,mode:'created'};}));
app.post('/admin/reviews/:id/hide',admin,async(req,res)=>runAdminAction(req,res,'/admin#reviews',async()=>{const id=parseInt(req.params.id||0,10);if(!id){const e=new Error('후기 ID가 올바르지 않습니다.');e.status=400;throw e;}const r=await q("UPDATE reviews SET status='hidden' WHERE id=$1 AND status='visible' RETURNING id,title,vendor_id",[id]);if(!r.rows[0]){const e=new Error('공개 상태의 후기를 찾을 수 없습니다.');e.status=409;throw e;}const x=r.rows[0],v=await q('SELECT name FROM vendors WHERE id=$1',[x.vendor_id]);await logAdmin(req,'후기 숨김','review',id,`${v.rows[0]?.name||'업체 없음'} / ${x.title||'-'} / visible → hidden`);}));
app.post('/admin/reviews/:id/show',admin,async(req,res)=>runAdminAction(req,res,'/admin#reviews',async()=>{const id=parseInt(req.params.id||0,10);if(!id){const e=new Error('후기 ID가 올바르지 않습니다.');e.status=400;throw e;}const r=await q("UPDATE reviews SET status='visible' WHERE id=$1 AND status='hidden' RETURNING id,title,vendor_id",[id]);if(!r.rows[0]){const e=new Error('숨김 상태의 후기를 찾을 수 없습니다.');e.status=409;throw e;}const x=r.rows[0],v=await q('SELECT name FROM vendors WHERE id=$1',[x.vendor_id]);await logAdmin(req,'후기 공개','review',id,`${v.rows[0]?.name||'업체 없음'} / ${x.title||'-'} / hidden → visible`);}));
app.post('/admin/delete/:table/:id',admin,async(req,res)=>runAdminAction(req,res,req.params.table==='vendors'?'/admin#vendors':'/admin#'+req.params.table,async()=>{const allowed={vendors:'vendors',banners:'banners',users:'users',reviews:'reviews',notices:'notices',events:'events',inquiries:'inquiries'}; const table=allowed[req.params.table]; const id=parseInt(req.params.id||0,10); if(!table||!id)throw new Error('삭제 대상이 올바르지 않습니다.'); if(table==='users'){const u=await q('SELECT role FROM users WHERE id=$1',[id]); if(u.rows[0]?.role==='admin'){const error=new Error('admin_account_managed_in_settings');error.status=403;throw error;}} if(table==='vendors'){const r=await q('UPDATE vendors SET status=$1 WHERE id=$2 RETURNING id',['inactive',id]); if(!r.rows[0])throw new Error('업체를 찾을 수 없습니다.'); await logAdmin(req,'업체 비활성화','vendors',id,'관리자 삭제 대신 비활성화'); return;} if(table==='banners'){const r=await q('DELETE FROM banners WHERE id=$1 AND vendor_id IS NULL RETURNING id,title',[id]);if(!r.rows[0]){const e=new Error('삭제 가능한 관리자 직접 배너를 찾을 수 없습니다.');e.status=409;throw e;}await logAdmin(req,'배너 삭제','banner',id,r.rows[0].title||'');return;} if(table==='reviews'){const r=await q("WITH old AS (SELECT id,title,vendor_id,status old_status FROM reviews WHERE id=$1 AND status IN ('visible','hidden') FOR UPDATE),updated AS (UPDATE reviews r SET status='deleted' FROM old WHERE r.id=old.id RETURNING r.id,old.title,old.vendor_id,old.old_status) SELECT * FROM updated",[id]);const x=r.rows[0];if(!x){const e=new Error('삭제 가능한 후기를 찾을 수 없거나 이미 삭제되었습니다.');e.status=409;throw e;}const v=await q('SELECT name FROM vendors WHERE id=$1',[x.vendor_id]);await logAdmin(req,'후기 삭제','review',id,`${v.rows[0]?.name||'업체 없음'} / ${x.title||'-'} / ${x.old_status} → deleted`);return;} const r=await q(`DELETE FROM ${table} WHERE id=$1 RETURNING id`,[id]); if(!r.rows[0])throw new Error('삭제 대상을 찾을 수 없습니다.'); await logAdmin(req,'삭제',req.params.table,id,'관리자 삭제');}));
