...
app.get('/admin/api/notices',admin,async(req,res)=>{
  try{
    ...
    if(qText){params.push(`%${qText}%`);const p=`$${params.length}`;where.push(`(n.id::text ILIKE ${p} OR n.title ILIKE ${p} OR n.content ILIKE ${p})`);}
    if(['true','1','pinned'].includes(pinned)){params.push(true);where.push(`n.is_pinned=$${params.length}`);}else if(['false','0','normal'].includes(pinned)){params.push(false);where.push(`n.is_pinned=$${params.length}`);}
    ...
    const {page,limit,offset}=adminPageParams(req),rows=await q(`SELECT n.id,n.title,n.content,n.is_pinned,n.created_at FROM notices n${whereSql} ORDER BY ${orderBy} LIMIT $${params.length+1} OFFSET $${params.length+2}`,[...params,limit,offset]);
    const count=await q(`SELECT COUNT(*) FROM notices n${whereSql}`,params),total=Number(count.rows[0]?.count||0),summary=await q(`SELECT COUNT(*)::int total,COUNT(*) FILTER (WHERE is_pinned=true)::int pinned,COUNT(*) FILTER (WHERE COALESCE(is_pinned,false)=false)::int normal,COUNT(*) FILTER (WHERE created_at>=CURRENT_TIMESTAMP-INTERVAL '7 days')::int recent FROM notices`);
